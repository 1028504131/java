				...
				<div class="row placeholders">
					<div>
						<button type="button" class="btn btn-default show-add-form"
							data-toggle="modal" data-target="#role-form-div">添加新部门</button>
					</div>
					...
					<table class="table table-hover table-bordered role-list">
						...
						<c:forEach items="${page.resultList}" var="userGroup">
							<tr>
								...
								<td class="roleid">${userGroup.groupId }</td>
								<td>${userGroup.groupName }</td>
								<td>${userGroup.groupDesc }</td>
								<td>${userGroup.groupCode }</td>
								<td>${userGroup.groupState eq 0 ? '禁用':'启用' }</td>
								<td>
									<button type="button" class="btn btn-warning ">禁用</button>
									<button type="button" class="btn btn-primary ">更改权限</button>
								</td>
							</tr>
						</c:forEach>
					</table>
		...
		<div class="modal-dialog modal-md" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="role-form-title">添加部门</h4>
				</div>
				<div class="modal-body">
					<form class="role-form">
						...
						<div class="form-group">
							<label for="groupNameInput">名称</label> <input type="text"
								name="roleName" class="form-control" id="NameInput"
								placeholder="角色名">
						</div>
						<div class="form-group">
							<label for="descInput">描述</label> <input type="text"
								name="groupDesc" class="form-control" id="descInput"
								placeholder="角色描述">
						</div>
						<div class="form-group">
							<label for="codeInput">代码</label> <input type="text"
								name="groupCode" class="form-control" id="codeInput"
								placeholder="角色代码">
						</div>
					</form>
				</div>
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary userGroup-submit"></button>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="modal fade " id="delete-confirm-dialog" tabindex="-1"
		...
		<div class="modal-dialog modal-sm" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">确认删除所选部门吗?</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
					<button type="button"
